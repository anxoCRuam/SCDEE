...
from apps.exams.models import PageProfile, ZoneType
...
from apps.instances.models import ExamPage, PageIssueType, PageStatus
...
def recognize_page(page: ExamPage) -> dict:
    ...
    # Step 1: Run QR recognition across all candidate profiles.
    ...
    if qr_result and qr_result.value and qr_result.value.get("valid"):
        ...
        # This is the earliest point where org_id is known.
...
def _run_qr_recognition(image_bytes: bytes) -> RecognitionResult | None:
    ...
    In production, this would crop to QR zone coordinates first.
    For robustness, we also try the full image.
    """
...
def _crop_zone(image_bytes: bytes, zone) -> bytes | None:
    ...
    try:
        ...
        # Assume the image covers the full page.
        # PDF origin is bottom-left; image origin is top-left.
        # We need the page dimensions from the profile.
...
def _assign_organization(page: ExamPage, org_id: str) -> None:
    ...
    The page arrives with organization=None because the watcher has
    no per-directory org mapping. The QR payload carries the org_id
    that was embedded when the instrumented PDF was generated.
    """
