...
1. Archiving the previous course (is_active=False).
...
from apps.notifications.models.notifications import Notification, NotificationStatus
...
def _execute_transition(
    *,
    ...
    previous_course.save(update_fields=["is_active", "updated_at"])
    ...
    # Step 5: Archive non-archived exam instances.
...
def _archive_instances(course: AcademicCourse) -> int:
    ...
    # Two-step to avoid Django bug with lazy FK resolution on multi-table
    # JOIN subqueries in UPDATE: exam→exams.Exam is a lazy string ref that
    # may not be resolved if apps.exams hasn't been imported yet.
    instance_ids = list(
        ExamInstance.unfiltered.filter(
            exam__subject__course=course,
        ).values_list("pk", flat=True)
    )
    if not instance_ids:
        return 0
    return ExamInstance.unfiltered.filter(pk__in=instance_ids).update(
