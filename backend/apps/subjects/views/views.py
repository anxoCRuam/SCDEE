...
def _can_manage_subject(user, subject) -> bool:
    """Check if user can manage a subject (manager, or coordinator)."""
    if user.is_staff or user.is_superadmin:
        return True
    return subject.coordinator_id == user.pk
...
class GroupListCreateView(APIView):
    ...
    @extend_schema(
        ...
        summary="List groups in a subject",
    )
    def get(self, request: Request, subject_pk: str) -> Response:
        ...
        return Response(GroupResponseSerializer(groups, many=True).data)

    @extend_schema(
        tags=["Groups"],
        request=CreateGroupSerializer,
        responses={201: GroupResponseSerializer},
        summary="Create group in a subject",
    )
...
class MemberListCreateView(APIView):
    ...
    @extend_schema(
        ...
        summary="Assign user to subject",
    )
    def post(self, request: Request, subject_pk: str) -> Response:
        subject = _get_subject_or_404(subject_pk, request.user)
        if not subject:
...
class MemberDetailView(APIView):
    ...
    @extend_schema(
        tags=["Members"],
        summary="Remove member from subject",
        responses={204: None},
    )
...
class MembershipPermissionsView(APIView):
    ...
    @extend_schema(
        ...
        summary="Get effective permissions (RF-5.5)",
    )
    ...
    @extend_schema(
        ...
        responses={200: EffectivePermissionsSerializer},
        summary="Update permission overrides (RF-5.3)",
    )
...
class MySubjectsView(APIView):
    ...
    @extend_schema(
        ...
        summary="List my subjects in the active course",
    )
