...
from apps.annotations.serializers.annotations import (
...
from apps.annotations.services.annotations import (
...
class AnnotationListCreateView(APIView):
    ...
    def post(self, request: Request, instance_pk: str) -> Response:
        ...
        payload_str = serializer.validated_data.get("payload", "")

        # Obtener bytes según el tipo
        if annotation_type == AnnotationType.AUDIO:
            ...
            content_bytes = request.FILES["file"].read()
        else:
            if not payload_str:
                return Response(
        ...
        # Extraer problem_id opcional
        problem_id = (
            str(serializer.validated_data["problem_id"])
            if serializer.validated_data.get("problem_id")
            else None
        )

        try:
            problem = Problem.objects.get(pk=problem_id) if problem_id else None
        except ExamInstance.DoesNotExist:
            return Response({"error_code": "NOT_FOUND"}, status=status.HTTP_404_NOT_FOUND)

        if not can_user_grade_instance(request.user, instance, problem):
...
class AnnotationDetailView(APIView):
    ...
    @extend_schema(
        ...
        summary="Update annotation (RF-10.2) owner or OrgManager",
    )
    def put(self, request: Request, annotation_pk: str) -> Response:
        ...
        content_bytes = request.FILES["file"].read() if "file" in request.FILES else None
        payload_str = serializer.validated_data.get("payload", "")
        content_bytes = payload_str.encode("utf-8") if payload_str else content_bytes
        ...
        try:
            annotation = update_annotation(
                ...
                if serializer.validated_data.get("problem_id") is not None
                else None,
                content_bytes=content_bytes,
            )
        except AnnotationServiceError as exc:
    ...
    @extend_schema(
        tags=["Annotations"],
        summary="Delete annotation (RF-10.3) owner or OrgManager",
        responses={204: None},
    )
...
class OCRGradeView(APIView):
    ...
    def post(self, request: Request, annotation_pk: str) -> Response:
        ...
        try:
            result = ocr_grade_annotation(annotation, request.user)
        except AnnotationServiceError as exc:
            return Response({"error_code": exc.code}, status=status.HTTP_400_BAD_REQUEST)
...
class OCRStylusView(APIView):
    ...
    def post(self, request: Request, annotation_pk: str) -> Response:
        ...
        try:
            result = ocr_stylus_annotation(annotation, request.user)
        except AnnotationServiceError as exc:
            return Response({"error_code": exc.code}, status=status.HTTP_400_BAD_REQUEST)
