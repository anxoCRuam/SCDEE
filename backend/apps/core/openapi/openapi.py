...
the postprocessing hook in :mod:`apps.accounts.spectacular`, so views
...
# ════════════════════════════════════════════════════════════════════
# OpenApi examples reused across multiple endpoints
# ════════════════════════════════════════════════════════════════════


PAGINATED_RESPONSE_EXAMPLE = OpenApiExample(
    name="PaginatedListExample",
    summary="Paginated list envelope (offset-based, see RNF-15)",
    description=(
        "All list endpoints return the same envelope. ``count`` is the "
        "total number of matching items, ``next``/``previous`` are full "
        "URLs (or null), and ``results`` is the page slice."
    ),
    value={
        "count": 247,
        "next": "https://api.scdee.example.com/api/v1/users/?limit=25&offset=25",
        "previous": None,
        "results": [
            {"id": "00000000-0000-0000-0000-000000000000", "first_name": "…", "last_name": "…"},
        ],
    },
    response_only=True,
)
