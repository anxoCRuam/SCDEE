...
def test_fast_health_does_not_probe_minio_or_celery(monkeypatch):
    ...
    monkeypatch.setattr("apps.core.services.health.check_minio", _boom)
    monkeypatch.setattr("apps.core.services.health.check_celery", _boom)
...
def test_deep_health_includes_minio_and_celery_components(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.core.services.health.check_minio",
        lambda: {"status": "healthy"},
    )
    monkeypatch.setattr(
        "apps.core.services.health.check_celery",
        lambda: {"status": "healthy", "workers": 1},
    )
