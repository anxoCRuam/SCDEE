...
def check_assembling_complete(instance: ExamInstance) -> bool:
    ...
    received_count = instance.pages.exclude(status=PageStatus.DISCARDED).count()

    if received_count >= instance.expected_pages:
...
def update_instance(
    ...
    *,
    ...
    if "model_id" in data:
        from apps.exams.models import ExamModel
...
def bulk_publish(exam, *, notify_students: bool = True) -> dict[str, Any]:
    ...
    """
    from apps.notifications.services import notify_grades_published
...
def compose_instance_pdf(
    ...
    *,
    ...
    Downloads each page from MinIO. Pages may be either PDFs (for legacy
    or PDF-native uploads) or raster images (PNG/JPEG) coming straight
    from the scanner; both are normalised to a PDF page before merging.

    When ``watermark_for_student`` is true, raster pages are stamped with
    a dynamic watermark identifying the student before being embedded
    (RF-16.6). Already-PDF pages are merged as-is, since rasterising and
    re-encoding them would be lossy and is out of scope for v1.0.
    """
