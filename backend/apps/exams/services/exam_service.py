...
def update_exam(exam: Exam, *, data: dict) -> dict[str, Any]:
    ...
    if "date" in data and data["date"] != exam.date:
        changes["date"] = {"old": exam.date, "new": data["date"]}
        exam.date = data["date"]
...
def create_page_profile(
    *,
    ...
    if exam_model.blank_pdf_pages == 0:
        raise ExamServiceError(
            code="NO_BLANK_PDF",
            detail="You must upload a blank PDF before creating page profiles.",
        )
    if page_number < 1 or (
    ...
    page_width = 0.0
    page_height = 0.0
    for dim in exam_model.page_dimensions:
        if dim["page"] == page_number:
            page_width = float(dim["width"])
            page_height = float(dim["height"])
            break
    else:
        raise ExamServiceError(
            code="MISSING_PAGE_DIMENSIONS",
            detail=f"Page dimensions for page {page_number} not found in cached metadata. "
            "Re-upload the blank PDF to refresh metadata.",
            field="page_number",
        )
...
) -> list[RecognitionZone]:
    """Create one or more identical zones across pages.

    If `page_numbers` is provided, each number must correspond to an
    existing `PageProfile` of the same model.  The zone is replicated
    on every such profile.

    Returns the list of created zones (one per page).
    """
    model = page_profile.exam_model

    targets: list[PageProfile] = []
    if page_numbers:
        profiles = {p.page_number: p for p in PageProfile.objects.filter(exam_model=model)}
        for num in page_numbers:
            try:
                targets.append(profiles[num])
            except KeyError:
                raise ExamServiceError(
                    code="INVALID_PAGE_NUMBER",
                    detail=f"Page {num} has no profile in this model.",
                    field="page_numbers",
                ) from None
    else:
        targets = [page_profile]

    # Validate zone parameters once
...
def set_convocation(
    *,
    ...
    for student_id in student_ids:
        ...
        if not membership:
            invalid_entries.append({"user_id": student_id, "reason": "NOT_ACTIVE_STUDENT"})
        elif not membership.group:
            invalid_entries.append({"user_id": student_id, "reason": "NO_GROUP_ASSIGNED"})
        else:
            valid_students.append(student_id)

    # Conjuntos para diff
    current_ids = set(
        ExamConvocation.objects.filter(exam=exam).values_list("student_id", flat=True)
    )
    new_ids = {uuid.UUID(s) for s in valid_students}  # Convertir a UUID para comparar
    ...
    with transaction.atomic():
        ...
        if to_remove:
            ExamConvocation.objects.filter(exam=exam, student_id__in=to_remove).delete()
        # Insertar los nuevos
        if to_add:
            ExamConvocation.objects.bulk_create(
                [ExamConvocation(exam=exam, student_id=sid) for sid in to_add]
            )
