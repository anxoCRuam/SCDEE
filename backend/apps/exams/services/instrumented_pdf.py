...
from apps.exams.models import ExamModel, PageProfile, ZoneType
...
def store_instrumented_pdf(model: ExamModel, pdf_bytes: bytes) -> str:
    """Store the generated instrumented PDF in MinIO and update the model.
    ...
    exam = model.exam
    key = f"{exam.organization_id}/exams/{exam.pk}/models/{model.pk}/instrumented.pdf"

    upload_to_minio(key=key, data=pdf_bytes, content_type="application/pdf")

    model.instrumented_pdf_ref = key
    model.instrumented_pdf_valid = True
    model.save(
        update_fields=[
            "instrumented_pdf_ref",
            "instrumented_pdf_valid",
            "updated_at",
        ]
    )
