...
from apps.accounts.authentication import AuthenticationError, TokenPayload, get_auth_plugin
...
from apps.core.openapi.errors import ErrorCode, error_response, validation_error_response
from apps.core.utils.throttling import LoginRateThrottle
...
class LoginView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "``is_superadmin`` and ``is_staff`` so the multi-tenant middleware can resolve the "
...
class LogoutView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        refresh_token = serializer.validated_data["refresh_token"]
        access_token = None
        if hasattr(request, "auth") and isinstance(request.auth, TokenPayload):
            # Podemos extraer el token crudo del header
            auth_header = request.META.get("HTTP_AUTHORIZATION", "")
            if auth_header.startswith("Bearer "):
                access_token = auth_header[7:]

        plugin = get_auth_plugin()
        plugin.revoke(refresh_token, access_token=access_token)
