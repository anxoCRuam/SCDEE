...
\chapter{Estado del arte\label{CAP:STATE-OF-THE-ART}}
  ...
  \section{Asignación automatizada de documentos a identidades%
           \label{SEC:SOTA-MATCHING}}%
          {chapters/02-state-of-the-art/24-matching}
...
\chapter{Diseño\label{CAP:DESIGN}}{chapters/04-design/intro}
  ...
  \section[Arquitectura por capas]%
          {Arquitectura por capas con \textit{Service Layer}%
           \label{SEC:DES-LAYERS}}%
          {chapters/04-design/42-architecture-layers}
  \section[Modelo de datos y multi-tenant]%
          {Modelo de datos y aislamiento \dfn{multi-tenant}%
           \label{SEC:DES-DATA}}%
          {chapters/04-design/43-data-model}
  \section[Plugins intercambiables]%
          {Plugins intercambiables\label{SEC:DES-PLUGINS}}%
          {chapters/04-design/44-plugins}
  \section[Pipeline de ingesta y reconocimiento]%
          {\textit{Pipeline} de ingesta y reconocimiento%
           \label{SEC:DES-INGEST}}%
          {chapters/04-design/45-pipeline}
  \section[Ciclo de vida de las instancias]%
          {Ciclo de vida de las instancias de examen%
           \label{SEC:DES-LIFECYCLE}}%
          {chapters/04-design/46-lifecycle}
  \section{Seguridad y cumplimiento\label{SEC:DES-SECURITY}}%
          {chapters/04-design/47-security}
  \section[\acs{api} \acs{rest} y contrato de integración]%
          {\acs{api} \acs{rest} y contrato de integración%
           \label{SEC:DES-API}}%
...
\chapter{Diagramas de secuencia\label{AP:SEQUENCE}}%
        {appendices/G-sequence-diagrams/G-overview}
  \section{Autenticación y contexto multi-tenant%
           \label{SEC:SEQ-AUTH}}%
          {appendices/G-sequence-diagrams/G1-auth}
  \section{Ingesta multipágina y pipeline completo%
           \label{SEC:SEQ-INGEST}}%
          {appendices/G-sequence-diagrams/G2-ingest}
  \section{Flujo de revisión%
           \label{SEC:SEQ-REVIEW}}%
          {appendices/G-sequence-diagrams/G3-review}
