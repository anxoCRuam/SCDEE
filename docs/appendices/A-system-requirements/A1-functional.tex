\begin{functional}
  ...
  \begin{functional}
    ...
    \textbf{Proceso:} el sistema realiza las siguientes acciones de forma transaccional: (1) marca el curso anterior como archivado (is\_active=False, status=ARCHIVED); (2) desactiva (is\_active=False) a todos los usuarios no gestores de la organización; (3) marca todas las SubjectMembership asociadas al curso archivado como is\_active=False; (4) archiva las notificaciones del curso anterior; (5) establece todas las instancias de examen al estado archivado, independientemente de su estado actual. Se registra la operación completa en el log de auditoría.
  ...
  | Permiso | COORD | TEACHER | STUDENT | |---------|-------|---------|---------| | can\_create\_exams | Sí | No | No | | can\_manage\_models | Sí | No | No | | can\_manage\_call\_list | Sí | No | No | | can\_assign\_correctors | Sí | No | No | | can\_grade | Sí | Sí | No | | can\_annotate | Sí | Sí | No | | can\_resolve\_issues | Sí | No | No | | can\_publish\_grades | Sí | No | No | | can\_manage\_reviews | Sí | No | No | | can\_view\_all\_instances | Sí | No | No | | can\_export\_grades | Sí | No | No | | can\_manage\_members | Sí | No | No | | can\_delegate\_perms | Sí | No | No |
  ...
  \begin{functional}
    ...
    El sistema debe permitir al coordinador o a un profesor con el permiso can\_manage\_call\_list definir la lista de alumnos convocados a un examen.
    ...
    \textbf{Proceso:} el sistema verifica que el modelo tiene al menos un perfil de página con al menos una zona de tipo QR. Para cada página del PDF en blanco, superpone un código QR en las coordenadas de cada zona QR definida. El contenido del QR codifica `org\_id`, `exam\_id`, `model\_id` y `page\_number` en formato JSON minificado con un checksum de detección de errores. El QR generado tiene el tamaño exacto de la zona definida. El PDF instrumentado resultante se almacena en MinIO como un objeto independiente del PDF en blanco original, sin modificar este último. Los metadatos de generación (fecha, versión) se almacenan en base de datos. Si existía un PDF instrumentado previo para ese modelo, se sobreescribe.

    \textbf{Salida:} respuesta con la confirmación de la generación y la referencia al PDF instrumentado en MinIO.
    ...
    \textbf{Entrada:} petición GET al recurso de descarga del PDF instrumentado del modelo.

    \textbf{Proceso:} el sistema verifica que existe un PDF instrumentado válido (no invalidado) para el modelo. Genera una URL temporal con expiración hacia el fichero en MinIO. Si el PDF instrumentado está invalidado (por modificación de zonas QR o del PDF en blanco), responde con un error indicando que debe regenerarse primero.

    \textbf{Salida:} respuesta con la URL temporal para la descarga del PDF instrumentado, o error HTTP 409 si el PDF instrumentado está invalidado.
  ...
  \begin{functional}
    ...
    \item \textbf{Registro de plugins de reconocedores.}

    El sistema debe permitir registrar y mantener varios motores de OCR como plugins intercambiables, así como los reconocedores de QR y CHECKBOX como componentes fijos del sistema.

    \textbf{Entrada:} configuración del sistema que define los motores OCR disponibles (por ejemplo, EasyOCR, Tesseract, servicios externos) y los reconocedores fijos de QR y CHECKBOX.

    \textbf{Proceso:} cada motor OCR implementa la interfaz `BaseOCREngine` que acepta una imagen y devuelve el texto reconocido con un nivel de confianza. Los reconocedores de QR y CHECKBOX implementan la interfaz `BaseRecognizer` genérica con el mismo contrato `recognize(image\_bytes) → (value, confidence)`. Los motores OCR se registran con un identificador único, nombre descriptivo y parámetros propios (idioma, modelo entrenado, etc.). Los reconocedores QR y CHECKBOX no son configurables por organización: el sistema utiliza siempre la implementación fija.
    ...
    \item \textbf{Fallback de proximidad temporal para páginas sin QR.}

    El sistema debe asociar temporalmente las páginas sin QR legible a la instancia más reciente del mismo escáner o sesión de ingesta, como mecanismo de fallback para exámenes de longitud variable.

    \textbf{Entrada:} evento interno disparado cuando una página no puede identificarse mediante QR (incidencia QR\_READ\_ERROR).

    \textbf{Proceso:} el sistema registra la marca temporal de recepción de la página y la identidad del origen (carpeta SFTP o sesión de ingesta). Busca la instancia cuya última página llegó más recientemente desde el mismo origen. Si existe una instancia candidata en estado ASSEMBLING y la diferencia temporal es inferior al umbral configurable (por defecto, 30 segundos), asocia la página huérfana a esa instancia con incidencia `ORPHAN\_PAGE`. Si no existe candidato, la página queda en estado ORPHAN sin instancia asignada, para gestión manual.

    \textbf{Salida:} página asociada por proximidad con incidencia ORPHAN\_PAGE activa, o página en estado ORPHAN pendiente de gestión manual.
